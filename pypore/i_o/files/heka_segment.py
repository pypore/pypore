...
class HekaSegment(FileSegment):
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            ...
            new_slice = slice_combine(self._get_total_dimension_len(), self._slice, item)

        return HekaSegment(self.filename, _slice=new_slice,
                           _sample_rate=sample_rate, _channel_selected=channel_selected)
    ...
    def __iter__(self):
        ...
        indices = self._slice.indices(self._get_total_dimension_len())
        n_elements = get_slice_length(self._get_total_dimension_len(), self._slice)
    ...
    def _get_total_dimension_len(self):
    ...
    def shape(self):
        if self._shape is None:
            ...
            #else:
            x = get_slice_length(self._get_total_dimension_len(), self._slice)
            self._shape = (x,)
        return self._shape
